...
if [[ "$DATASET" == "Pet_Supplies" ]]; then
    DROPOUT=0.3         # 119K users 資料量大，降低正則化讓模型充分學習（預設 0.4 過度正則化）
    MILESTONES="50,100" # 第一次 run Ep35 附近到頂，Ep50 降 LR 合理
    PATIENCE=75         # 預設值，確保能等到 Ep100 的第二次 LR drop
fi

if [[ "$DATASET" == "Office_Products" ]]; then
    MILESTONES="60,120"  # 第一次 run 在 Ep36 就到頂，MILESTONES=40 太早截斷學習；改為 Ep60 才降 LR
    LR_GAMMA=0.5         # 修正：0.3 會降到 0.00015（反而更大幅），保持預設 0.5（降到 0.00025）
fi
