...
# ============================================================
# === ABLATION: 消融變體 Patch 函數 ===
# ============================================================
ABLATION_VARIANTS = ['noSpec', 'shareProj', 'noDual', 'singleGraph', 'noOrtho']

def apply_ablation_variant(variant, model, args):
    """
    根據消融變體，在模型建立後、訓練開始前，對模型做執行時修改。
    不會改動原始的 models/*.py 檔案。
    """
    if variant == 'noOrtho':
        # 移除正交解耦損失：lambda_spec = 0
        args.lambda_spec = 0.0
        print(f"[Ablation-noOrtho] Set lambda_spec = 0.0")

    elif variant == 'noSpec':
        # 移除譜濾波器：將 spectral_disentangler 替換為回傳 zeros 的 dummy
        original_sd = model.spectral_disentangler

        class DummySpectralDisentangler(torch.nn.Module):
            def __init__(self, item_num, emb_dim):
                super().__init__()
                self.item_num = item_num
                self.emb_dim = emb_dim

            def forward(self, x, adj_self, adj_dele):
                zeros = torch.zeros_like(x)
                return zeros, zeros

        model.spectral_disentangler = DummySpectralDisentangler(
            model.item_num, model.emb_dim
        ).to(next(model.parameters()).device)
        print(f"[Ablation-noSpec] Replaced spectral_disentangler with zeros-output dummy")

    elif variant == 'shareProj':
        # 共用投影層：proj_cor 指向 proj_sim（共用同一組 MLP 參數）
        model.proj_cor = model.proj_sim
        print(f"[Ablation-shareProj] proj_cor now shares parameters with proj_sim")

    elif variant == 'noDual':
        # 合併雙通道 Transformer：cor 通道使用 sim 通道的 Transformer 和 IntentCapture
        enc = model.sequential_encoder
        enc.cor_transformer = enc.sim_transformer
        enc.cor_intent_capture = enc.sim_intent_capture
        print(f"[Ablation-noDual] cor_transformer & cor_intent_capture now share with sim channel")

    elif variant == 'singleGraph':
        # singleGraph 的處理在圖建構階段（main 函數中），不在這裡
        print(f"[Ablation-singleGraph] Graph merging applied (see graph construction)")
...
def main():
    ...
    # === ABLATION: 消融變體選擇 ===
    parser.add_argument('--variant', type=str, required=True, choices=ABLATION_VARIANTS,
                        help=f'消融變體：{ABLATION_VARIANTS}')
    ...
    if not args.test_only:
        # === ABLATION: checkpoint 路徑加入 variant 名稱 ===
        checkpoint_dir = f"./checkpoints/{args.dataset}/ablation_{args.variant}_{timestamp}"
    ...
    # === ABLATION: singleGraph 合併兩張圖為同一張 ===
    if args.variant == 'singleGraph':
        merged_edges = torch.cat([sim_edges, com_edges], dim=1)
        if sim_weights is not None and com_weights is not None:
            merged_weights = np.concatenate([sim_weights, com_weights])
        elif sim_weights is not None:
            merged_weights = np.concatenate([sim_weights, np.ones(com_edges.shape[1], dtype=np.float32)])
        else:
            merged_weights = None
        sim_edges = merged_edges
        com_edges = merged_edges
        sim_weights = merged_weights
        com_weights = merged_weights
        print(f"[Ablation-singleGraph] Merged sim+cor edges → {merged_edges.shape[1]} total edges for BOTH channels")
    ...
    # === ABLATION: 套用消融變體 ===
    model, args = apply_ablation_variant(args.variant, model, args)
